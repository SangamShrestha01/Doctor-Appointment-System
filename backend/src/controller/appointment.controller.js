...
export const bookAppointment = asyncHandler(async (req, res, next) => {
    ...
    // ✅ FIX #1: Case-insensitive Map key lookup
    ...
    if (!availableSlots.includes(time)) {
        return next(
            new AppError(400, `Time slot "${time}" not available on ${weekday}`)
        );
    }

    // ✅ FIX #2: Use the matched mapKey for consistency
    ...
    // Find the next occurrence of this weekday (always future)
    while (
