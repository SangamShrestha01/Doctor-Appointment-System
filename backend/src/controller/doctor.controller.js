  ...
  "user",
  "name email image address"  // ✅ include address for edit profile
  );
  ...
  // ✅ Update name & address on User model
  if (name || address) {
  await User.findByIdAndUpdate(currentUser._id, {
  ...(name && { name }),
  ...(address && { address }),
  ...
  // ✅ Upsert DoctorProfile — create if doesn't exist, update if it does
  const doctorFields = {
  ...
  { user: currentUser._id },
  { $set: doctorFields },
  { new: true, upsert: true } // ✅ creates profile if not found
  );
